...
## 十一、主配方优先语义(用户裁决 2026-08-25,记录为未来项)

**v1 现状**:某物品的联产物(如配方15产氢)可免费满足该物品需求,min Σx 下若联产充足则其主配方执行 0 次。当该物品本身是盈余物品(联产已超需求)时,此为预期行为(本来就不需要生产)。

**用户裁决**:当物品为**净需求**时,应"主配方优先"——需求由该物品唯一主配方满足,其他配方联产物供给不计入其需求满足(仅作盈余展示)。这是 v2 需求,v1 **不实现**。

**明确禁止**:不用"多余产物最小化"作为 v1 目标函数——它会牺牲 min Σx 效率(为省多余副产品而改变生产方案)。多余产物最小化是 v2 可选配平策略,届时随目标函数参数化一并设计。

**实现提示(v2)**:主配方优先可在构模层实现(如:净需求物品的守恒行拆为主配方供给项 + 联产盈余项),或目标函数注入主配方权重。具体方案留待 v2 设计。
