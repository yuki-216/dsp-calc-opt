...
// { "目标物品": "氢", "建筑数量": 0, "配方id": 1, "增产剂等级": 0, "增产模式": 0, "建筑": 0 }
...
function NplRow({row, set_row, remove_row}) {
    ...
    function get_output_num(item, recipe, building_scale, pro_mode, pro_num, building_name) {
        ...
        let pro_lv = Number(pro_num);
        if (pro_lv >= game_data.proliferator_effect.length) pro_lv = game_data.proliferator_effect.length - 1;
        let proliferator_data = game_data.proliferator_effect[pro_lv];
        if (pro_mode == 2) {
    ...
    let output_num = get_output_num(item, recipe, row["建筑数量"] * selected_building["倍率"], row["增产模式"], row["增产剂等级"], selected_building["名称"]);
    return <tr className="table-info">
        ...
        {/* 所选增产剂 */}
        <td><ProNumSelect choice={row["增产剂等级"]} onChange={set_row_prop("增产剂等级", true)}/></td>
        <td>
            {/* 所选工厂种类 */}
