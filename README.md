...
- **多 mod 数据源** — 顶部导航栏切换「原版 / 创世之书 / 星环」数据源（选择持久化，方案/需求独立存取）；mod 引入的新机制（负熵翻倍配方、新配方类别、新发电/采集体系等）无需改动计算引擎
  - **创世之书**（GenesisBook）— 扩展型，数据格式与原版一致
  - **星环**（OrbitalRing）— 改造型：移除 50 个原版物品（含能量矩阵/宇宙矩阵，换成自己的矩阵体系）、新增 5 类设施与单一等级增产剂；297 物品 / 384 配方
  - 新增 mod 的流程见 [添加 Mod 指南](docs/添加mod指南.md)
- **LP 失败回退** — 求解失败时弹窗提示并自动回退到上次成功计算的配方选择，避免卡在计算失败状态
...
- **在新窗口计算** — 输出表物品旁点击按钮，原页面视为原矿，新标签页独立计算该物品生产链（继承点击前的原矿化列表）
- **新窗口** — 导航栏按钮，在新窗口打开一个不附带需求的空白计算页
- **子窗口沙盒** — 「新窗口计算」/「新窗口」开出的标签页不写回共享的本地缓存，改动只存在自己标签页里（刷新可恢复），不会覆盖原窗口保存的配置
- **配置管理** — 顶部工具栏入口，把本地方案/需求/设置/矿物可用量/原矿化/数据源/优化策略导出为 JSON 文件或从文件导入（按配置项合并）
